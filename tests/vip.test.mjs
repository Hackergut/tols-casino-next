...
test("the VIP ladder has 50+ ranks and the published XP thresholds", () => {
  ...
  assert.match(src, /\["Seed", "seed", 500, 0\.9\]/);
  assert.match(src, /\["Copper 1", "copper", 1_000, 0\.9\]/);
  assert.match(src, /\["Iron 1", "iron", 10_000, 9\]/);
  assert.match(src, /\["Amber 1", "amber", 100_000, 80\]/);
  assert.match(src, /\["Onyx 1", "onyx", 17_000_000, 9_360\]/);
  assert.match(src, /\["Celestial", "celestial", 1_000_000_000, 0\]/);
  const ranks = [...src.matchAll(/\["[^"]+", "[a-z]+", [0-9_]+, [0-9_.]+\]/g)];
  assert.ok(ranks.length >= 50, `expected 50+ ranks, found ${ranks.length}`);
});

test("rank-up panel bonuses are the published fixed amounts", () => {
  const src = read("src/lib/vip.ts");
  assert.match(src, /rankUpBonus/);
  assert.match(src, /\["Garnet 1", "garnet", 5_800_000, 2_700\]/);
  const service = read("src/lib/vip-service.ts");
  assert.match(service, /vipTier\(newLevel\)\.rankUpBonus/);
});
