          ...
          {formatRtpHistory().length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={formatRtpHistory()} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                ...
                <Tooltip contentStyle={{ background: "#1a1b20", border: "1px solid #333", borderRadius: 8, fontSize: 12 }} formatter={(v, _n, p) => {
                  const payload = (p as { payload?: { game?: string; reason?: string } })?.payload;
                  return [`${v}% (${payload?.game ?? "—"})`, payload?.reason || "RTP"];
                }} />
