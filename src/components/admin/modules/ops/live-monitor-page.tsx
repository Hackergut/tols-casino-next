      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            icon: AlertTriangle,
            color: (summary?.active_alerts ?? 0) > 0 ? "var(--color-loss, #ff4d5e)" : "#4ade80",
          },
          {
            label: "Wagered 24h",
            value: fmtAmount(summary?.wagered_24h ?? 0),
            icon: DollarSign,
            color: "var(--color-lime, #ccff00)",
            prefix: "\u20bf ",
          },
          {
            label: "House Edge 24h",
            value: `${summary?.house_edge_24h ?? "0.00"}%`,
            icon: TrendingUp,
            color: parseFloat(summary?.house_edge_24h ?? "0") > 0 ? "#4ade80" : "var(--color-loss, #ff4d5e)",
          },
        ].map((s, i) => (
          ...
          {stats?.house_edge_by_game && stats.house_edge_by_game.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats.house_edge_by_game} layout="vertical" margin={{ left: 16, right: 32 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "#1a1b20", border: "1px solid #333", borderRadius: 8, fontSize: 12 }}
                ...
                <Bar dataKey="actual_house_edge_pct" name="Actual Edge" radius={[0, 4, 4, 0]}>
                  {stats.house_edge_by_game.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={Number(entry.actual_house_edge_pct) >= Number(entry.house_edge) ? "#4ade80" : "var(--color-loss, #ff4d5e)"}
                    />
                  ))}
                </Bar>
            ...
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              ...
              {(stats?.unresolved_alerts?.length ?? 0) > 0 && (
                <Badge variant="destructive" className="ml-auto text-xs">
                  {stats?.unresolved_alerts?.length}
                </Badge>
              )}
            </CardTitle>
          </CardHeader>
                      ...
                      {stats.unresolved_alerts.map((a) => (
                        ...
                        >
                          ...
                          <TableCell className="py-2">
                            <Badge variant={(ALERT_COLORS[a.alert_type] as "default" | "destructive" | "secondary" | "outline") || "outline"} className="text-xs capitalize">
                          ...
                          <TableCell className="py-2 text-right">
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-6 text-xs px-2 text-green-400 hover:text-green-300"
                      ...
                      <TableRow key={s.id} className="border-b border-white/5">
                        ...
                        <TableCell className="text-center py-2">
                          {s.current_win_streak > 0 ? (
                            <span className="text-green-400 font-bold text-sm">{s.current_win_streak}</span>
                          ) : <span className="text-muted-foreground text-xs">&mdash;</span>}
                        </TableCell>
                        <TableCell className="text-center py-2">
                          {s.current_loss_streak > 0 ? (
                            <span className="text-red-400 font-bold text-sm">{s.current_loss_streak}</span>
                          ) : <span className="text-muted-foreground text-xs">&mdash;</span>}
                        </TableCell>
                      </TableRow>
                    ))}
          ...
          {formatRtpHistory().length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={formatRtpHistory()} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "#1a1b20", border: "1px solid #333", borderRadius: 8, fontSize: 12 }}
                  formatter={(v: number, _n: string, p: { payload: { game: string; reason: string } }) => [
                    `${v}% (${p.payload.game})`,
                    p.payload.reason || "RTP"
                  ]}
                />
                    ...
                    {stats.recent_results.slice(0, 50).map((r) => (
                      ...
                      >
                        ...
                        <TableCell className="text-xs tabular-nums py-1.5 text-right">
                          {r.win_amount > 0 ? <span className="text-green-400">{"\u20bf"}{fmtAmount(r.win_amount)}</span> : <span className="text-muted-foreground">&mdash;</span>}
                        </TableCell>
                        <TableCell className="py-1.5 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <Badge
                              ...
                              style={r.outcome === "win" ? { background: "rgba(74,222,128,0.15)", color: "#4ade80", border: "1px solid rgba(74,222,128,0.3)" } : {}}
                            >
                              {r.outcome}
                            </Badge>
                            {r.forced_outcome && (
                              <Badge variant="outline" className="text-xs px-1 py-0 border-yellow-500/40 text-yellow-400">
                                forced
                              </Badge>
                            )}
                          </div>
