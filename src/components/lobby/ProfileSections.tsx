...
function VipSection({ onBack }: { onBack: () => void }) {
  ...
  const w = wallet.data;
  const level = w?.vipLevel ?? 0;
  const xp = w?.xp ?? 0;
  const nextAt = (level + 1) * 1000;
  const pct = Math.min(100, (xp % 1000) / 10);
  const perks = ["Faster withdrawals", "Weekly cashback", "Dedicated host", "Exclusive drops", "Higher limits"];
  return (
    <Shell title="VIP" subtitle="Your tier, progress and perks" icon={Crown} onBack={onBack}>
      <div className={CARD} style={CARD_STYLE}>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.35)" }}>Current Tier</p>
            <p className="mt-1 text-3xl font-black text-vip">VIP Level {level}</p>
          </div>
          <Crown className="w-10 h-10 text-vip" />
        </div>
        <div className="mt-4">
          <div className="flex justify-between text-xs mb-1" style={{ color: "rgba(255,255,255,0.4)" }}>
            <span>{xp} XP</span><span>Next: {nextAt} XP</span>
          </div>
          <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
            <div className="h-full rounded-full" style={{ width: `${pct}%`, background: "linear-gradient(90deg, var(--color-win), var(--color-lime))" }} />
          </div>
        </div>
      </div>
      <div className={CARD} style={CARD_STYLE}>
        <p className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>Perks</p>
        <div className="grid gap-2 sm:grid-cols-2">
          {perks.map((p, i) => (
            <div key={p} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm" style={{ background: "rgba(255,255,255,0.03)", color: i < level ? "var(--color-lime)" : "rgba(255,255,255,0.4)" }}>
