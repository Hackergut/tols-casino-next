...
2. Verifica next.config.ts CORS su /api/platform/:path* verso TOWER_HOST (da GOVERNANCE_TOWER_URL=https://gov.tols.fun)
...
- **tolsgovernz** → Settings → Environment Variables → Production → incolla BLOCCO 1 (GOVERNANCE_TOWER_URL=https://gov.tols.fun, APP_URL=URL Casino, GOVERNANCE_BRIDGE_SECRET, PLATFORM_JWT_PRIVATE_KEY)
